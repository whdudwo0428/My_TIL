...
- **제 10회 AI/SW 융합 해커톤**, **SW 중심대학 디지털 경진대회** 준비
  - 비접촉 생체신호 기반 공유 모빌리티 사용자 인증 시스템
    - 공유 모빌리티 어플리케이션을 사용할 때 회원인증 초기에만 신분 등록하고, 이후 실제 사용시 추가적인 인증과정이 없어 악용의 우려가 있음
    - 대여 직턴, rPPG 모델을 사용하여 생체신호를 측정하고, 건강상태, 음주상태를 추정하여 정상 기준치를 벗어났을 때 이용 제한을 할 수 있는 UI를 개발
      - 제 10회 AI/SW 융합 해커톤 : 07-10~07-11
...
- LG AImers Academy : [AI Essential Course]
  ...
  - 07-03~07-04 : KAIST 신진우 교수님, 『Mathematics for ML』 수강
   ...
   - 07-04~07-06 : 연세대학교 노알버트 교수님, 『지도학습』 수강
    ...
    - 07-06~07-13 : 고려대학교 이병준 교수님, 『강화학습』 수강
      - 정규학기 수업 강화학습 인강 복습
      - MDP and Planning
      ...
      - Deep Q Learning
      - Policy Gradient
        - 어려워서 포기함 당장 강화학습 실험 할 생각도 없어서 다른것부터... 녹화해두고 나중에 공부할때 다시 하자  

    - 07-14~07-15 : 서울대학교 이상학 교수님,『Causality』수강
      - 인과성과 인과추론 기본 개념
      - 인과추론을 위한 두가지 프레임워크
      - 인과추론을 위한 다양한 방법과 특징
 
    - 07-16~07-18 : 서울대학교 강필성 교수님, 『시계열데이터 및 AI모델 성능 최적화』수강
      - 순환신경망 기반의 시계열 데이터 회귀
      - 합성곱 기반의 시계열 데이터 회귀
      - 트랜스포머 기반의 시계열 데이터 회귀

    - 07-21~07-22 : 서울대학교 문일경 교수님, 『SCM과 수요 예측』 수강
      - 공급망 관리 기본 개념
      - 수요 예측 기법
      - 수요 예측 실습 및 사례 분석
